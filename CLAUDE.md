...
- Handoff to students: once a teacher has a `spreadsheetId`, the server encrypts `{spreadsheetId, refreshToken}` directly into the student link's `token` query param (`lib/studentToken.ts`) — this is the entire "student database," a link is the credential. `/api/teacher/sheet/link` mints it.
...
**Route map:**
- `/teacher`, `/teacher/setup` — teacher-only pages (OAuth login, Google Picker or template auto-creation, get the student link). `app/teacher/AboutButton.tsx` is the shared version/about-info modal.
